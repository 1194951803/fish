...
- **真实待办**：内置可用的待办事项面板，支持添加、删除、拖拽排序，数据持久化到本地文件
- **网页嵌入面板**：侧边栏嵌入任意网页，支持三种加载模式：
  - **直接模式**：传统 iframe 嵌入
  - **代理模式**：通过内置反向代理绕过 X-Frame-Options / CSP 嵌入限制，自动重写资源路径
  - **阅读模式**：自动提取网页正文（标题+正文+图片），渲染为干净的阅读视图
...
5. **代理模式**：反向代理内置 SSRF 防护，禁止访问内网地址
6. **阅读模式**：基于文本密度分析自动提取正文，不依赖 BeautifulSoup 等第三方库
